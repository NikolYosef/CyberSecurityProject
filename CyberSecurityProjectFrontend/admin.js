...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("RefreshAI").addEventListener("click", loadAiReport);
  document.getElementById("ApplyAI").addEventListener("click", loadAiReport);
  loadEquipmentAdmin();
  loadUsers();
  loadOrders();
});
...
async function loadAiReport() {
  ...
  const onlyEl = document.getElementById("aiOnlyAnomalies");
  const tbody = document.getElementById("aiTbody");

  const raw = limitEl ? limitEl.value.trim() : "";
  const limit = raw ? parseInt(raw, 10) : 10;
  const onlyAnomalies = onlyEl ? (onlyEl.value === "1") : true;

  const res = await fetch(API_URL + `/admin/ai/analyze?limit=${encodeURIComponent(limit)}`, {
    headers: getAuthHeaders()
  });

  if (!res.ok) {
    return setMsg("Load AI report failed: " + await res.text());
  }

  const rows = await res.json();
  tbody.innerHTML = "";

  const filtered = onlyAnomalies ? rows.filter(r => r.is_anomaly) : rows;

  if (!filtered.length) {
    const tr = document.createElement("tr");
    tr.innerHTML = `<td colspan="6">No results</td>`;
    tbody.appendChild(tr);
    return;
  }
